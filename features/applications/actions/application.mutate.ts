...
import type { Application, ApplicationCreate, ApplicationStatus, EmployerSetStatus } from "../schema"
...
export function useSetApplicationStatus() {
  ...
  return useMutation({
    mutationFn: ({ id, status }: { id: string; status: ApplicationStatus | EmployerSetStatus }) =>
      apiClient<Application, { status: ApplicationStatus | EmployerSetStatus }>(`/applications/${id}`, {
        method: "PATCH",
        body: { status },
      }),
    onSuccess: async () => {
