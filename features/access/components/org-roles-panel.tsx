...
import { sanitizePermissions, type PermissionMap } from "@/connector/access/catalog"
import {
    ...
    <div className="grid gap-4">
      ...
      <ul className="grid gap-3">
        {RESERVED_ORG_ROLES.map((role) => (
          <li key={role} className="grid gap-2 rounded-md border border-border p-3">
            <div className="flex items-center justify-between gap-2">
              <p className="font-medium">{role}</p>
              <span className="text-muted-foreground text-xs">{t("builtinRole")}</span>
            </div>
            <PermissionSummary permissions={BUILTIN_ORG_PERMISSIONS[role] ?? {}} />
          </li>
        ))}
        {roles.map((role) => (
          <li
